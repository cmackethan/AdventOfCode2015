...
def sumGrid(grid: list[list[bool]]) -> int:
    sum = 0
    for row in grid:
        for cell in row:
            if cell:
...
def updateGrid(grid: list[list[bool]], instr: Instruction) -> list[list[bool]]:
...
def solve(file):
    grid: list[list[bool]] = [[False for i in range(1000)] for j in range(1000)]
